...
<?php include('connect_db.php');
								$query = mysqli_query($link,"SELECT diaPago, idCredito, idClienteCredito, credito.codigoProducto, descrProducto, CONCAT( d.DescrDia,  ': ', cantidadCredito, ' col ', descProducto) AS descripcion
FROM dia d    
INNER JOIN credito ON d.numDia = credito.diaPago
INNER JOIN cliente ON credito.idClienteCredito = cliente.idCliente  INNER JOIN persona on cliente.idPersona = persona.idPersona
WHERE credito.saldoActual >0 order by d.numDia");
								while($row = mysqli_fetch_assoc($query)){?>
