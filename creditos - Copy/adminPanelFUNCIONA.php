						    ...
						    <div class="form-group">
						      ...
						      <div class="col-sm-8">
						       <SELECT NAME="selClienteCombo" SIZE=1 > 
								...
								$query = mysqli_query($link,"SELECT CONCAT(u.usuario,' ', a.cantidadAbono,' ',a.idCobrador,' ',c.descrProducto ) AS credit,  a.idAbono 
FROM abono a
INNER JOIN credito c ON a.idCreditoAbono = c.idCredito INNER JOIN usuario u on u.idUsuario= c.idVendedor");
								while($row = mysqli_fetch_assoc($query)){?>
								<option value=" <?php echo $row['idAbono'] ?> "><?php echo $row['credit']; ?></option>
								<?php } ?>
						    ...
						    <div class="form-group">
						      <label class="col-sm-4 control-label" for="email">Monto Total</label>
						      <div class="col-sm-8">
							 ...
							 <SELECT NAME="selClienteCombo" SIZE=1 > 
								  ...
								  <?php include('connect_db.php');
							  $query = mysqli_query($link,"select SUM(a.cantidadAbono) as total  from abono a inner join credito c on a.idCreditoAbono=c.idCredito");
...
<div class="form-group">
						      ...
						      <div class="col-sm-8">
						         <SELECT NAME="selCombo" SIZE=1 required> 
							   	<?php include('connect_db.php');
								$query = mysqli_query($link,"SELECT CONCAT(SUM( a.cantidadAbono ), u.usuario) AS credit, u.idUsuario
FROM abono a
INNER JOIN credito c ON a.idCreditoAbono = c.idCredito
INNER JOIN usuario u ON u.idUsuario = c.idVendedor
GROUP BY u.idUsuario");
						    ...
						    <div class="form-group">
						      <label class="col-sm-4 control-label" for="name">Total</label>
						      <div class="col-sm-8">
						         <SELECT NAME="selCombo" SIZE=1 required> 
							   	<?php include('connect_db.php');
								$query = mysqli_query($link,"SELECT SUM( cantidadAbono ) AS total, idCobrador
FROM abono
GROUP BY idCobrador");
								while($row = mysqli_fetch_assoc($query)){?>
