...
<li><a href="moduloClientesLimpios.php"><i class="service-icon fa fa-envelope"></i>&nbsp;Clientes limpios</a></li>
    </ul>
  </div><!-- /.navbar-collapse -->
  </div>
</nav>
						    ...
						    <div class="form-group">
						      ...
						      <div class="col-sm-8">
						       <SELECT NAME="selClienteCombo" SIZE=1 > 
								...
								$query = mysqli_query($link,"SELECT CONCAT( a.fechaAbono,  ' Cob: ', u.usuario,  ': ', a.cantidadAbono,  ':', persona.nombre  ) AS credit, a.idAbono
FROM abono a
  ...
  ");
						    ...
						    <div class="form-group">
							 ...
							 <SELECT NAME="selClienteCombo" SIZE=1 > 
								  ...
								  <?php include('connect_db.php');
							  $query = mysqli_query($link,"select SUM(a.cantidadAbono) as total  from abono a inner join credito c on a.idCreditoAbono=c.idCredito WHERE a.fechaAbono >= CURRENT_DATE
...
<div class="form-group">
						      ...
						      <div class="col-sm-8">
						      <SELECT NAME="selCombo"  SIZE=1 onchange="fetch_select(this.value, <?php echo $_SESSION['idTipoClientes'];?>);">
...
WHERE  credito.diaPago between '0' and '13' AND cliente.balance >0 and credito.cantidadCredito>credito.montoPagado and anulada='0'
order by   persona.nombre");
 ...
 <div class="form-group">
						      ...
						      <div class="col-sm-8">
						        <textarea placeholder="Aqui la cuota" class="form-control" name="cuota" id="cuota" rows="1"></textarea>
						      </div>
						    </div>
...
<?php include('connect_db.php');
								...
								$query = mysqli_query($link,"SELECT distinct idCredito, idClienteCredito, credito.codigoProducto, sum(cliente.balance) as balance, descrProducto,  CONCAT( 'Persona:', credito.idCredito, ' ', persona.nombre, cantidadCredito,  ' col.', credito.observacion,  ' cuota.', credito.cuotaCredito) AS descripcion
FROM dia d
INNER JOIN credito ON d.numDia = credito.diaPago
INNER JOIN cliente ON credito.idClienteCredito = cliente.idPersona
INNER JOIN persona ON cliente.idPersona = persona.idPersona
WHERE balance <1 and  anulada='0'
 ...
 <div class="form-group">
    ...
    <div id="services">
      <div class="container">
        <div class="row">
		<div class="col-md-4 col-md-offset-4 text-center">
            <h2>Control de morosidad de clientes</h2>
			<hr>
          </div>
          <div class="col-md-5 col-md-offset-3">
		  <!-- contact form starts -->
...
<div class="form-group">
						      <label class="col-sm-4 control-label" for="name">Lista de clientes ordenados por monto pendiente de pago</label>
						      <div class="col-sm-8">
						         <SELECT NAME="selCombo" onchange="cargaDatosCliente(this)" SIZE=1 required id="selectMoroso"> 
							   <?php include('connect_db.php');
								$query = mysqli_query($link,"SELECT cliente.idCliente, persona.telefono1, persona.telefono2, concat(persona.nombre,' ',persona.primerapellido,': ', cliente.balance) as nombreCompleto  from cliente inner join persona on cliente.idPersona= persona.idPersona order by cliente.balance desc");
								while($row = mysqli_fetch_assoc($query)){?>
								<option value=" <?php echo $row['telefono1'] + '|' + $row['telefono2'];  ?> "><?php echo $row['nombreCompleto']; ?></option>
								<?php } ?>
						    ...
						    <div class="form-group">
						      <label class="col-sm-4 control-label" for="cel"  id="cel">Número celular 1</label>   <form id="cont-form" method="post" action="www.grupowtjj.com/marcasgenuinas/sendmail.php">
<fieldset>
    <textarea id="message" name="message" placeholder="FASTNESS le recuerda que tiene pagos pendientes, favor comunicarse al 40305859" rows="3" cols="40">FASTNESS le recuerda que tiene pagos pendientes, favor comunicarse al 40305859
    </textarea>
   
    
</fieldset>
<fieldset>
    <input class="button" id="submit" name="submit" type="submit" value="Cobrar!">
</fieldset>
</form>
                            ...
                            <div class="form-group">
                              <div class="col-sm-8">
						        <label class="col-sm-4 control-label" for="cel2"   id="cel2">Número celular 2</label>   <form id="cont-form" method="post" action="www.grupowtjj.com/marcasgenuinas/sendmail.php">>
<fieldset>
